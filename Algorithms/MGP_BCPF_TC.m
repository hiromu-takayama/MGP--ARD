...
del_a_gamma=zeros(R,1);
del_b_gamma=zeros(R,1);

a_del=a_del0;
%0.4
delta=a_del.*ones(R,1);
...
for n=1:N
    tau_para = tau_para + diag(Z{n}'*Z{n});
end

tau_para=0.1*mean(tau_para);
...
gammas=a_gamma0./(tau_para*tau);

%tau_para=1;

%1e-6 (Iが小さい場合，1e-3とかに数値をあげる)
...
if verbose >1,
    ...
    hori = figure('Position',[scrsz(3)*0.2 scrsz(4)*0.3 scrsz(3)*0.6 scrsz(4)*0.4]);
    set(0,'CurrentFigure',hori);
    switch verbose,
        case 3,
    ...
    set(findall(hori,'type','text'),'fontSize',12);
    drawnow;
end


% draw mse and lambda(orignal)
h_pro = figure;
figure(h_pro);
...
for it=1:maxiters
    ...
    for n=1:N
        subplot(N+1,1,n); heatmap(Z{n},'GridVisible','off');
    end
    
    if R > 1 && max(abs(1./gammas)) > 0
        subplot(N+1,1,N+1); bar(abs(1./gammas));  %axis([0 R 0 max(abs(1./gammas))])
    else    
        subplot(N+1,1,N+1); bar(abs(1./gammas)); 
    end
    
    if it < 7
        saveas(h1, "mgp_core"+num2str(it-1)+".png");
    end
    ...
    %% Update latent tensor tau
    for r=1:R
        tmp = gammas(r:R).*(tau(r:R)./delta(r));
        del_a_gamma(r)=a_del+(R-r+1)*a_gamma0(r);
        del_b_gamma(r)=1 + tau_para.*sum(tmp);
        delta(r) = del_a_gamma(r)./del_b_gamma(r);
        
        tau = cumprod(delta);
    end

    %% Update hyperparameters gamma
    a_gammaN = a_gamma0 + (0.5*sum(dimY))*ones(R,1);
    b_gammaN = 0;
    for n=1:N
    ...
    b_gammaN = tau_para*tau + 0.5.* b_gammaN;
    ...
    % temp3 is written (original)
    temp3 = sum(-safelog(gamma(a_gamma0)) + a_gamma0.*cumsum(psi(del_a_gamma)-safelog(del_b_gamma)) -  cumprod(del_a_gamma./del_b_gamma).*(a_gammaN./b_gammaN) + (a_gamma0-1).*(psi(a_gammaN)-safelog(b_gammaN)));
    ...
    %temp8-10 (original)
    temp8 = sum(-safelog(gamma(a_gamma0)) + a_gamma0.*cumsum(psi(del_a_gamma)-safelog(del_b_gamma))+(a_gamma0-1).*(psi(a_gammaN)-safelog(b_gammaN)) - cumprod((a_gammaN./b_gammaN).*(del_a_gamma./del_b_gamma)) ); 
    temp9 = sum( -safelog(gamma(a_del))+(a_del-1)*(psi(del_a_gamma)-safelog(del_b_gamma)) - del_a_gamma./del_b_gamma );
    temp10 = sum( safelog(gamma(del_a_gamma)) - (del_a_gamma-1).*psi(del_a_gamma) - safelog(del_b_gamma) + del_a_gamma);

    LB(it) = temp1 + temp2 + temp3 + temp4 + temp5 + temp6 + temp7+ temp8 + temp9 + temp10;
    ...
    if DIMRED==1  && it >=2,
        if R~= max(rankest)
            ...
            % effevtive prune (original)
            delta=delta(1:R);
            del_a_gamma=del_a_gamma(1:R);
            del_b_gamma=del_a_gamma(1:R);
            a_gamma0=a_gamma0(1:R);
            gammas=gammas(1:R);
        end
    end
