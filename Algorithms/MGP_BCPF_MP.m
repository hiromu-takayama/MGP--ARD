...
if verbose >2,
    ...
    subplot(2,3,4); bar(1./gammas); title('Posterior mean of \lambda'); xlabel('Latent components'); ylabel(''); axis tight;
...
for it=1:maxiters
    ...
    if R > 1 && max(abs(1./gammas)) > 0
        subplot(N+1,1,N+1); plot(abs(1./gammas));  axis([0 R 0 max(abs(1./gammas))])
    else    
        subplot(N+1,1,N+1); plot(abs(1./gammas)); 
    end
    ...
    %temp8-9 (original)
    temp8 = sum( -safelog(gamma(a_del))+(a_del-1)*(psi(del_a_gamma)-safelog(del_b_gamma)) - del_a_gamma./del_b_gamma );
    ...
    % draw mse and lambda(orignal)
    set(0,'CurrentFigure',h_pro);
    figure(h_pro);
    err_trace(it)=sqrt(sum(err(:)))/norm(Y(:));
    subplot(2,1,1);plot(1:it,err_trace(1:it));
    xlabel('iteration');  
    ylabel('mean-squared error');
    subplot(2,1,2);plot(sort(abs(1./gammas),'descend'));
    xlabel('lambda');
    drawnow;     
